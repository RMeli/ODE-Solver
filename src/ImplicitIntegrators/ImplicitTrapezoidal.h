...
public:
   ...
   */
  ImplicitTrapezoidal(double (*ODE_)(double, double),
                      double dx_,
  ...
  //! Destructor.
  /*!
   Needed an explicit definition of the destructor, in order to delete the
   pointer to ITFunction class.
   */
  ~ImplicitTrapezoidal();
...
private:
  ...
  /*!
   ...
   This class is defined as a nasted class of the ImplicitTrapezoidal integrator
   in oder to keep private the nonlinear function to solve, while using a
   general NewtonMethod class that works on any Function (this is needed because
   a pointer to a function is different from a pointer to a member function). A
   class defined as nasted class is automatically a friend class of the
   container class (ImplicitEuler in this case) and therefore we can access
   directly the private members of the containign class. This is useful as the
   nonlinear function needs private members of ImplicitTrapezoidal (a pointer to
   the ImplicitIntegrator is stored inside the ITFunction class).
   */
  class ITFunction : public Function {
  public:
    //! Constructor.
    /*!
     Take a pointer to an ImplicitTrapezoidal integrator. This link is needed
    ...
    /*!
     Derivative of the nonlinear function defining the implicit trapezoidal
     method (needed by NewtonMethod)
     */
    double df(double);
  ...
  private:
    ...
    /*!
     ...
     the ImplicitTrapezoidal.

     No destructor needed because ImplcitiTrapezoidal destruction is handled by
     the ImplicitSolver.
     */
  ...
  /*!
   The ITFunction class is defined here as a nasted class. However, obviously,
   an instance of the class is needed and this pointer is used to store an
   IEFunction object: we have a double link between ITFunction and
   ImplicitTrapezoidal integrator (each class points to the other). A the
   ITFunction is initialized inside the constructor of the ImplicitTrapezoidal
   with the this pointer, no mismatch is allowed.
   */
  ITFunction* ITF;
};
